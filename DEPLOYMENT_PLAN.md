...
| 11b | Tab close mid-recording (source/Mac, browser) | beforeunload warning; connection record closed (`end_reason=tab_closed`) | 🧑 KRIS — **not scriptable**: needs a live serial connection for a connection record to exist. In a real browser `beforeunload` *does* warn and the `pagehide` beacon should be delivered (the server survives the tab), so this path may still produce `tab_closed`. The row-11 server-side close covers it either way. |
